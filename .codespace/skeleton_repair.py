...
def heal(data: bytes, budget: int = 2000, min_unknown_pct: float = 0.0):
    ...
    while attempts < budget:
        parsed, reader, err = parse_once(units, forced)
        attempts += 1
        ...
        near = [i for i, u in enumerate(units) if isinstance(u, tuple) and i < base][-12:]
...
def main() -> int:
    ...
    ap.add_argument("--budget", type=int, default=400, help="лимит попыток перебора на файл")
    ...
    for path in targets:
        ...
        with open(path, "rb") as f:
            data = f.read()
        parsed, rep = heal(data, budget=args.budget)
